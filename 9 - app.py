...
with tab1:
    ...
    with col_input:
        with st.form("formulario_paciente"):
            ...
            peso = st.number_input("Peso (kg)", 30.0, 200.0, 70.0, step=0.1)
            
            st.markdown("---")
            st.markdown("**Hábitos e Histórico**")
            hist_fam = st.selectbox("Histórico Familiar de Obesidade?", ["sim", "nao"])
            favc = st.selectbox("Alimentos Calóricos Frequentes?", ["sim", "nao"])
            vegetais = st.selectbox("Consumo de Vegetais", ["raramente", "as vezes", "sempre"])
            refeicoes = st.slider("Quantidade de Refeições ao dia", 1, 4, 3)
            lanches = st.selectbox("Lanches entre refeições", ["nao consome", "as vezes", "frequentemente", "sempre"])
            fumante = st.selectbox("Fumante?", ["sim", "nao"])
            agua = st.slider("Água (Litros/dia)", 1, 3, 2)
            monitora = st.selectbox("Monitora Calorias?", ["sim", "nao"])
            atv_fisica = st.selectbox("Atividade Física", ["nenhuma", "1 a 2x/sem", "3 a 4x/sem", "5x/sem ou mais"])
            eletronicos = st.selectbox("Tempo de Tela", ["0 a 2h/dia", "3 a 5h/dia", "> 5h/dia"])
            alcool = st.selectbox("Álcool", ["nao bebe", "as vezes", "frequentemente", "sempre"])
            transporte = st.selectbox("Transporte", ["transporte publico", "a pe", "carro", "moto", "bicicleta"])
        ...
        if btn_calcular:
            if pipeline:
                ...
                input_df = pd.DataFrame({
                    'genero': [genero], 'idade': [idade], 'altura': [altura], 'peso_kg': [peso],
                    ...
                    'consumo_agua_dia': [agua], 'monitora_calorias': [monitora], 
            ...
            st.markdown("👈 **Preencha o formulário ao lado e clique em Calcular.**")
